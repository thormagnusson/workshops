...
void setup() {
  ...
  oscP5 = new OscP5(this, 14000);
    myRemoteLocation = new NetAddress("193.62.161.122",14000);
  flock = new Flock();
...
myMessage = new OscMessage("/mouse");
   myMessage.add(mouseX); /* add an int to the osc message */
   myMessage.add(mouseY); /* add an int to the osc message */
  oscP5.send(myMessage, myRemoteLocation);
  
  
  myMessage = new OscMessage("/flock");
   // myMessage.add(loc.y); /* add an int to the osc message */
...
class Boid {
 ...
 //     OscMessage myMessage = new OscMessage("/flock");
    myMessage.add(loc.y); /* add an int to the osc message */
 //   oscP5.send(myMessage, myRemoteLocation); 
  ...
  // Wraparound
  void borders() {
   // if (loc.x < -r) loc.x = 0;//width; //width+r;
   // if (loc.y < -r) loc.y = 0; //height; //height+r;
   // if (loc.x > width+r) loc.x = width; //-r;
   // if (loc.y > height+r) loc.y = height; //-r;
